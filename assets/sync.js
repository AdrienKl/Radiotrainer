/* =============================================================================
   Albatros VFR — ENREGISTREMENT DES SÉANCES EN BASE
   -----------------------------------------------------------------------------
...
(function(){
  ...
  function sansClefExercice(paquet, e){
    ...
    try{ console.warn('[Albatros VFR] exercice « ' + clef + ' » absent du catalogue : '
  ...
  function tenter(paquet, secondEssai){
    return pousser(paquet).then(
      ...
      function(e){
        ...
        try{ console.warn('[Albatros VFR] séance ' + (fatal ? 'REFUSÉE' : 'mise en attente') + ' :',
  ...
  window.addEventListener('rt:auth', function(ev){
    ...
    RTSync.reprise = RTSync.solderLesAbandons().then(function(){ return RTSync.viderLaFile(); })
      .then(function(n){ if (n) try{ console.info('[Albatros VFR] '+n+' séance(s) en attente envoyée(s).'); }catch(e){} });
  });
})();
